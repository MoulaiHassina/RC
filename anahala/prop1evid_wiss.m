function [BEL_Cdt_classique,t2] = prop1evid_wiss(nb_nodes,nb_parent_max)
...
%pour �viter les sous-graphes isol�s 
...
if length(list_nodes)==nb_nodes  %il s'agit d'un graphe connect�
...
%====Generation des distributions de possibilit�s====
ns=node_sizes(:)';
for d=1:nb_nodes
  distribution=[];
  sauv=[];
  ...
  if l==0  % noeud sans parent
     ...
     for v=1:(ns(d)-1)
        % g�n�ration d'un entier
        x=exp(-(str2num(int2str(rand(1)*10))));nbki=nbki+1;
        % g�n�ration de la distibution
        distribution = [distribution x]; 
      end   
        for i=1:2 
           fprintf(fid2,'%d \n',str2num(int2str(-log(distribution(i)))));
        end
        %generation des distributions avec un degr� maximal �gale � 1
  else 
     decalage=1;
     for m=1:l
...
% g�n�ration al�atoire d'une �vidence 
...
% g�n�ration al�atoire de la variable d'int�r�t
var_interest=var(4);
...
%prise en charge de l'�vidence par l'engine%
tic; % d�marrer la comptabilisation du temps de propagation
[engine] = global_propagation(engine, evidence); 
%prise en charge de la variable d'int�r�t%
marg = marginal_nodes(engine, var_interest);
BEL_Cdt_classique=marg.T(instance_interest);
t2=toc; % arr�t de la comptabilisation du temps de propagation
fprintf(fid1,'%f \n',BEL_Cdt_classique);
fprintf(fid1,'%f \n',t2); 
fprintf(fid3,'%d \n',nbki);
